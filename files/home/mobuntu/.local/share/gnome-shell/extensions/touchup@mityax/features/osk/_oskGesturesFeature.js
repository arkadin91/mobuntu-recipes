...
import { isKeyboardKey } from './_oskUtils.js';
...
class OSKGesturesFeature extends ExtensionFeature {
    ...
    onNewKeyboard(keyboard) {
        const recognizer = new GestureRecognizer({
            onGestureProgress: state => {
                if (settings.osk.gestures.swipeToClose.enabled.get()
            ...
            onGestureEnded: state => {
                ...
                else if (settings.osk.gestures.swipeToClose.enabled.get()
     ...
     */
    _selectReactiveChild(keyboard, point) {
        ...
        const candidates = findAllActorsBy(keyboard, (a) => a.mapped && a.reactive);
        const hitActor = candidates.find(key => key
